...
function showNumbersUrdu() {
    ...
    function dn(n) { return String(n).split('').map(d => URDU_DIGITS[parseInt(d)]).join(''); }
    function say(n) { speakUrdu(String(n)); }
    ...
    function showLevelPicker() {
        ...
        h += '<div class="card"><div class="title">🔢 اردو Urdu Numbers</div>';
        h += '<div class="inst">Pick a level:</div>';
        for (let i = 0; i < 5; i++) {
            const unlocked = i + 1 <= level, done = i + 1 < level;
            h += '<button class="' + (unlocked ? 'btn blue' : 'btn') + '" onclick="goNULevel(' + (i + 1) + ')" ' + (unlocked ? '' : 'disabled') + ' style="font-size:18px;padding:12px;margin:6px 0;width:100%">' + (done ? '✅' : unlocked ? '▶️' : '🔒') + ' Level ' + (i + 1) + ': ' + LEVEL_NAMES[i] + '</button>';
        }
        h += '</div>';
        document.getElementById('app').innerHTML = h;
    }

    window.goNULevel = function(l) { LEVELS[l - 1].type === 'learn' ? startLearn(l) : startPractice(l); };
    window.showNULevelPicker = showLevelPicker;
    ...
    function startLearn(lvl) {
        ...
        function flash() {
            ...
            let h = '<button class="back" onclick="showNULevelPicker()">← Back</button>';
            h += '<div class="card"><div class="title">🔢 Level ' + lvl + ': ' + LEVEL_NAMES[lvl - 1] + '</div>';
            h += '<div class="inst">Learn the Urdu numbers!</div>';
            ...
            h += '<div style="font-size:90px;font-weight:bold;direction:rtl;cursor:pointer" onclick="playNUFlash()">' + dn(n) + '</div>';
            h += '<div style="font-size:40px;color:#666;margin-top:10px">= ' + n + '</div>';
            h += '<button onclick="playNUFlash()" style="font-size:40px;background:none;border:none;cursor:pointer;margin-top:10px">🔊</button>';
            h += '</div><button class="btn green" onclick="nextNUFlash()" style="font-size:20px;padding:15px">Next →</button></div>';
            document.getElementById('app').innerHTML = h;
            startItemTimer();
            setTimeout(function() { say(n); }, 400);
        }

        window.playNUFlash = function() { say(teachNums[fIdx]); };
        ...
        function quiz() {
            ...
            let h = '<button class="back" onclick="showNULevelPicker()">← Back</button>';
            h += '<div class="card"><div class="title">🔢 Quiz: What number is this?</div>';
            ...
            h += '<div style="font-size:80px;font-weight:bold;direction:rtl;cursor:pointer" onclick="playNUFlash()">' + dn(n) + '</div>';
            h += '<button onclick="playNUFlash()" style="font-size:36px;background:none;border:none;cursor:pointer">🔊</button>';
            h += '</div><div class="choices">';
            choices.forEach(function(c) { h += '<button class="btn choice" onclick="pickNUQuiz(' + c + ',' + n + ',this)" style="font-size:28px;padding:15px;min-width:80px">' + c + '</button>'; });
            h += '</div></div>';
            document.getElementById('app').innerHTML = h;
            window.playNUFlash = function() { say(n); };
            setTimeout(function() { say(n); }, 300);
        }

        window.pickNUQuiz = function(picked, correct, btn) {
        ...
        function finishLevel() {
            ...
            let h = '<div class="card"><div class="title">🎉 Level ' + lvl + ' Complete!</div>';
    ...
    function startPractice(lvl) {
        ...
        function hearTap() {
            ...
            let h = '<button class="back" onclick="showNULevelPicker()">← Back</button>';
            h += '<div class="card"><div class="title">🔊 Hear & Tap</div>';
            h += '<div style="text-align:center;font-size:18px;color:#888">' + (qi + 1) + ' / ' + total + '</div>';
            h += '<div style="text-align:center;margin:20px 0">';
            h += '<button onclick="playNUP()" style="font-size:60px;background:none;border:none;cursor:pointer">🔊</button>';
            h += '<div style="font-size:18px;color:#888;margin-top:5px">Tap the number you hear</div>';
            h += '</div><div class="choices">';
            ch.forEach(function(c) { h += '<button class="btn choice" onclick="pickNUP(' + c + ',' + n + ',\'hear_tap\',this)" style="font-size:44px;padding:18px;min-width:90px;direction:rtl">' + dn(c) + '</button>'; });
            h += '</div></div>';
            document.getElementById('app').innerHTML = h;
            window.playNUP = function() { say(n); };
            setTimeout(function() { say(n); }, 400);
        }
        ...
        function findNum() {
            ...
            var ch = pickChoices(n, pool, 3);
            let h = '<button class="back" onclick="showNULevelPicker()">← Back</button>';
            h += '<div class="card"><div class="title">🔢 Find the Number</div>';
            h += '<div style="text-align:center;font-size:18px;color:#888">' + (qi + 1) + ' / ' + total + '</div>';
            h += '<div style="text-align:center;margin:20px 0">';
            h += '<div style="font-size:80px;font-weight:bold;direction:rtl;cursor:pointer" onclick="playNUP()">' + dn(n) + '</div>';
            h += '<button onclick="playNUP()" style="font-size:36px;background:none;border:none;cursor:pointer">🔊</button>';
            h += '<div style="font-size:18px;color:#888;margin-top:5px">What number is this?</div>';
            h += '</div><div class="choices">';
            ch.forEach(function(c) { h += '<button class="btn choice" onclick="pickNUP(' + c + ',' + n + ',\'find\',this)" style="font-size:28px;padding:15px;min-width:80px">' + c + '</button>'; });
            h += '</div></div>';
            document.getElementById('app').innerHTML = h;
            window.playNUP = function() { say(n); };
            setTimeout(function() { say(n); }, 300);
        ...
        function moreLess(dir) {
            ...
            let h = '<button class="back" onclick="showNULevelPicker()">← Back</button>';
            h += '<div class="card"><div class="title">' + (dir === 'more' ? '📈' : '📉') + ' Which is ' + label + '?</div>';
            h += '<div style="text-align:center;font-size:18px;color:#888">' + (qi + 1) + ' / ' + total + '</div>';
            h += '<div style="text-align:center;margin:20px 0">';
            ...
            h += '</div><div class="choices">';
            ch.forEach(function(c) { h += '<button class="btn choice" onclick="pickNUP(' + c + ',' + correct + ',\'' + dir + '\',this)" style="font-size:36px;padding:15px;min-width:90px;direction:rtl">' + dn(c) + ' <span style="font-size:16px;color:#888">(' + c + ')</span></button>'; });
            h += '</div></div>';
            document.getElementById('app').innerHTML = h;
            window.playNUP = function() { say(tgt); };
        }
        ...
        function closestQ() {
            ...
            let h = '<button class="back" onclick="showNULevelPicker()">← Back</button>';
            h += '<div class="card"><div class="title">🎯 Which is closest?</div>';
            h += '<div style="text-align:center;font-size:18px;color:#888">' + (qi + 1) + ' / ' + total + '</div>';
            h += '<div style="text-align:center;margin:20px 0">';
            ...
            h += '</div><div class="choices">';
            ch.forEach(function(c) { h += '<button class="btn choice" onclick="pickNUP(' + c + ',' + best + ',\'closest\',this)" style="font-size:36px;padding:15px;min-width:90px;direction:rtl">' + dn(c) + ' <span style="font-size:16px;color:#888">(' + c + ')</span></button>'; });
            h += '</div></div>';
            document.getElementById('app').innerHTML = h;
            window.playNUP = function() { say(n); };
        }
        ...
        window.pickNUP = function(picked, correct, mode, btn) {
            var ok;
        ...
        function finishP() {
            ...
            let h = '<div class="card"><div class="title">🎉 Practice Complete!</div>';
