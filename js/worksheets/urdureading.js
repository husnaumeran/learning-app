...
function showUrduReading() {
    ...
    function render() {
        ...
        html += '<div style="text-align:center;font-size:80px;margin:10px;font-family:serif;direction:rtl;cursor:pointer" onclick="speakUrdu(\''+l.sf+'\')">'+l.letter+'</div>';
        ...
        [{key:'fatha',label:'زَبَر',skey:'sf',color:'#FF6B35'},{key:'kasra',label:'زِیر',skey:'sk',color:'#00CC66'},{key:'damma',label:'پِیش',skey:'sd',color:'#0099FF'}].forEach(h => {
            html += '<div style="text-align:center;cursor:pointer;padding:10px 15px;border-radius:12px;background:#333" onclick="speakUrdu(\''+l[h.skey]+'\');speak(\''+l[h.skey]+'\');">';
