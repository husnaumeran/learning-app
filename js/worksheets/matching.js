...
function showMatchNumbers() {
    const level = Math.max(1, Number(getFocusNumber('match_numbers')) || 1);
    const pairs = generateMatchPairs(getDifficultyLevel('match_numbers'), level);
    ...
    function render() {
        ...
        pairs.forEach((_, i) => {
            ...
            const numCls = solved.has(nums[i])
                ? 'opacity:0.3'
                : (selected === nums[i] ? 'background:#fffbeb;border:3px solid #FF6B35' : '');
    ...
    window.selectEmoji = (n) => {
        ...
        recordResponse(
            'match_numbers',
            { type: 'match_numbers', number: selected, emoji_count: n },
            String(selected),
            String(n),
            correct,
            true,
        ...
        showFeedback(correct, () => {
            ...
            if (score === pairs.length) {
                completeWorksheet('Match Numbers', score, pairs.length);
                return;
            }
