...
- **2026-09-24** — **Interface polish is additive, by the owner's explicit instruction.** Asked for an Apple-style pass, then immediately: *"don't rewrite it"* and *"just use the elements to make it smother."* So `css/styles.css` keeps every original rule untouched and gains one appended block: press feedback on the controls that already exist, `touch-action: manipulation` and a transparent tap highlight (which remove the 300ms double-tap delay and the grey flash — the biggest felt change on a tablet), a 0.28s settle on `.card` because navigation is an `innerHTML` swap and every screen previously snapped in, and `prefers-reduced-motion` honoured. No colours, type, layout or class names changed. **Do not "finish the job" by restyling the app** — the constraint is deliberate, and the worksheets carry heavy inline styles that a redesign would fight. Note the `apple-design` skill is enabled on the owner's claude.ai account but is **not loadable from Claude Code**; the Skill tool rejects it by name and by id.
