...
1. **Ship the local client fixes.** The 2026-09-14 bug fixes are uncommitted. Commit and push once git identity and GitHub push auth work on this machine. Then delete the remote `qa` branch and disconnect Netlify.
2. **Owner applies the security migration** (`sql/migrations/20260914_01_security_hardening.sql`), then regenerate `sql/schema.sql` (§5 Ritual 3).
3. **Mastery — built and verified locally; waiting on the owner.** Client and migration are done; see `docs/MASTERY.md`. To go live, in this order: the owner applies migration 01, then 02, in the Supabase SQL editor; *then* push the client; then regenerate `sql/schema.sql`. **Do not push the mastery client before migration 02.** Nothing crashes, but until the migration exists it hides Numbers English/Arabic and Qaida levels that were stored only on the device, and the Qaida skip-ahead does nothing; the next login after the migration restores them. The earlier bug-fix batch has no such dependency and can ship first. *Background, kept for context:* Today "mastery" is decided in 9 places by 6 different rules, stored in two tables plus device-only localStorage, and the server system built for it (`child_skill_progress`) is never read. The evidence underneath is polluted: `recordPassiveResponse` (`helpers.js`) records tracing, videos and book pages as **correct first-try answers**, and `find_pairs` / `numbers_all` hardcode `is_correct = true`. Arabic Qaida records nothing, and Urdu Qaida records nothing below level 5. Direction: one server-side store, one engine in `finalize_session`, separate rules for scored skills and practice-only activities, and a migration that **never lowers a level a child has already reached** on any device. Folded in: complete `DOMAINS`; remove the `what_comes_next_letters` phantom (it runs the numbers worksheet and records under `what_comes_next_numbers`, so it looks "never practiced" forever); and delete the dormant, unreachable global floor in `getDifficultyLevel`.
4. **Refresh `PROJECT_PLAN.md`** against reality and the goals in §2.
