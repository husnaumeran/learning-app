...
- **2026-09-24** — **Interface polish is additive, by the owner's explicit instruction.** Asked for an Apple-style pass, then immediately: *"don't rewrite it"* and *"just use the elements to make it smother."* So `css/styles.css` keeps every original rule untouched and gains one appended block: press feedback on the controls that already exist, `touch-action: manipulation` and a transparent tap highlight (which remove the 300ms double-tap delay and the grey flash — the biggest felt change on a tablet), a 0.28s settle on `.card` because navigation is an `innerHTML` swap and every screen previously snapped in, and `prefers-reduced-motion` honoured. No colours, type, layout or class names changed. **Do not "finish the job" by restyling the app** — the constraint is deliberate, and the worksheets carry heavy inline styles that a redesign would fight. The `apple-design` skill loads as **`anthropic-skills:apple-design`** (the bare name is rejected, which is why the first pass went without it). Checked against it the same day, still inside the appended block only: taps no longer overshoot on release (Apple reserves bounce for gestures that carried momentum, and a tap carries none), reduced motion keeps fades and brightness feedback instead of killing everything, and `index.html` registers a passive `touchstart` listener because iOS Safari has only applied `:active` on touch when one exists — without it the press feedback may never show on an iPad.
- **2026-09-24** — **Letter joining built as rung 2 of `docs/LETTERS_TO_WORDS.md`** (`js/worksheets/joining.js`, skills `urdu_joining` / `arabic_joining`, migration `20260924_05`). The load-bearing detail: **a joined word is built by concatenating plain letter characters and letting the font's shaping engine choose the glyph.** Never concatenate the `initial`/`medial`/`final` strings — they carry a tatweel and are only valid as a single-shape label. "Separate" letters are rendered one `<span>` each so the shaping engine has no neighbour to connect them to; no ZWNJ needed. The file contains **no `while` or `do` loops at all**, which closes the rejection-sampling freeze class outright. `JOINING_SHAPE_FAMILIES` in that file is hand-written dot-family metadata (ب/پ/ت/ٹ/ث …) used to pick look-alike distractors, because the letter tables carry no skeleton field.
...
1. **Owner applies two migrations, then the client ships.** `sql/migrations/20260923_04_merge_color_patterns.sql` and `sql/migrations/20260924_05_letter_joining.sql`. Commit `006ce08` is committed but **deliberately not pushed** until they land. Pushing first degrades rather than breaks — `getBaseWeight` returns 2 for a skill missing from the `skills` table and the domain map skips it — but joining would sit at level 1 and never progress, and Color Patterns would lose its ladder. Afterwards regenerate `sql/schema.sql` (§5 Ritual 3); it is **stale**, predating migrations 02 and 03, so it does not show the mastery columns. Regenerate once, after both land.
